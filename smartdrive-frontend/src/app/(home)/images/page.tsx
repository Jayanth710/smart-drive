...
const ImagesPage = () => {
    const { imagesData, isLoading, error, refreshData } = useFetchCollections();
    ...
    if (error) {
        return (
            <div className="flex justify-center items-center flex-1 p-4 text-red-500">
                <p className="text-lg">Error: Could not load images.</p>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col gap-6 p-2">
            <div className="flex items-start justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-0.5">
                        Every image you&apos;ve uploaded, OCR&apos;d and indexed.
                    </p>
                </div>
            </div>
            ...
            <FileListWithDrawer
                files={inSearch ? searchResults : (imagesData || [])}
